...
從開機那一刻起就說中文——連 WAXWORKS 標題畫面下方，都疊上了「蠟像館之謎」：
...
### 方式 A：下載引擎包，把遊戲放進 `game/`（最推薦，免自己編）

到 [**Releases**](https://github.com/wicanr2/waxworks_dos_cht/releases) 下載你平台的**引擎包**（只含 patched ScummVM + 繁中字型，**不含遊戲**）：

| 平台 | 檔案 |
|---|---|
| Windows | `蠟像館之謎-CHT-windows-x64.zip` |
| macOS（arm64+Intel universal）| `蠟像館之謎-CHT-macOS-universal.dmg`（或 `.tar.gz`）|
| Linux | `蠟像館之謎-CHT-linux-x86_64.tar.gz` |

解開後，把你合法擁有的 Waxworks 原檔（`GAMEPC`、`TABLES01…`、`TEXT01…`、`ROOMS0x`、`*.VGA` 等**全部**）複製進裡面的 **`game/`** 資料夾，再依平台雙擊 `play-waxworks.bat`／`.command`／`.sh` 就能玩。想要原版 MT-32 配樂，把 `MT32_CONTROL.ROM`、`MT32_PCM.ROM` 也放進 `game/`。

📖 **完整圖文步驟見 [`docs/如何開始遊玩.txt`](docs/如何開始遊玩.txt)**（每個引擎包裡也內附一份 `README.txt`）。

### 方式 B：一鍵完整版（自己有遊戲檔者，最省事）
...
**三平台都到齊了**：Linux AppImage、Windows zip（全 DLL）、macOS universal（GitHub Action 雲端 lipo 產出）——全部內建原版 MT-32 配樂支援。推廣片、開發接續包（[`docs/DEV_SETUP.md`](docs/DEV_SETUP.md)）也備妥。
