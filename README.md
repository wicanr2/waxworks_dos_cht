...
Waxworks 是一款以「殘酷」聞名的老遊戲——**沒有自動地圖**、怪物埋伏、各種花式死法。當年多少人就是在這些沒有地圖的迷宮裡鬼打牆而放棄。

本繁中版加入了一個**動態自動地圖**：在第一人稱迷宮裡按 **`TAB`** 隨時叫出／收起，顯示你走過的迷宮佈局、牆壁與門、你目前所在的房間，以及**寶物**所在。

![動態地圖](docs/img/map.png)

技術上，Waxworks 的迷宮是「房間圖(graph)」——每個房間的 `roomExitStates`（16-bit，6 方向各 2 bits）記著哪面是牆、哪面是門。地圖從你所在房間 BFS 走出口、依方向位移佈成 2D 平面，把牆線與寶物即時畫進 hi-res 疊層。逆向分析見 [`docs/DYNAMIC_MAP_FEASIBILITY.md`](docs/DYNAMIC_MAP_FEASIBILITY.md)。

**怪物標記**：地圖上**紅點＝怪物、黃點＝寶物**（怪物是固定放置、非隨機生成，已逆向確認；以 `objectFlags==kOFText` 判定生物）。標記在走過的房間顯示。

### 友善模式（讓想純看劇情的玩家不被硬核戰鬥擋住）

| 熱鍵 | 功能 |
|---|---|
| **`TAB`** | 開／關動態地圖 |
| **`F5`** | 立即回復生命與心靈力 |
| **`F7`** | 切換**無敵模式**（生命／心靈力保持滿，不會死；右上角顯示「無敵模式」）|

Waxworks 以「花式死法」聞名——無敵模式讓你安心把四段故事看完，不必在同一個陷阱前死上二十次。

**規劃中**：怪物圖鑑補弗拉迪米爾 sprite、玩家朝向指北針、探索迷霧。

延伸文件（或許是華語圈第一份完整整理）：
- 📜 [**完整攻略 WALKTHROUGH.md**](docs/WALKTHROUGH.md)——四場景流程、謎題解法、BOSS 打法、經典死法。
- 👹 [**怪物圖鑑 BESTIARY.md**](docs/BESTIARY.md)——每種怪物的來歷、造型與反殺方式，含遊戲 sprite，為文化保留而作。
